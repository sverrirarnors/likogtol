...
factor(hmr.es.long, levels=c("Úrtak af stærð 10", "Úrtak af stærð 30", "Úrtak af stærð 100", "Úrtak af stærð 200"))

ggplot(hmr.es.long, aes(medaltal)) + geom_histogram() + facet_wrap(vars(staerd) ) + xlab("Meðaltal") + ylab("Fjöldi")
```
###e)
```{r}
ggplot(hmr.es.long, aes(medaltal)) + geom_histogram() + facet_wrap(vars(staerd), labeller = labeller(staerd=labels), scales = "free") + xlab("Meðaltal") + ylab("Fjöldi")
```
###f)
```{r}
