...
$$H_0: \mu_1=\mu_2$$ og $$H_1: \mu_1\neq\mu_2$$Þar sem $\mu_0$ er meðallengd á fyrra hafsvæðinu og $\mu_1$ meðallengd á seinna hafsvæðinu.

```{r}
t.test(h1$le,h2$le)
```

Hér er p-gildið 1.045e-07 þannig að ég get hafnað núlltilgátunni.Við getum því ályktað að meðaltal lengda á hafsvæðunum tveimur sé ekki það sama. 
...
```{r}
ggplot(data=h1,aes(factor(aldur),y=le))+geom_boxplot() + xlab("Aldur") + ylab("Lengd") + geom_smooth(method='lm', se=FALSE, aes(group=1))+geom_smooth(method="loess",se=FALSE, aes(group=1),color="red")
